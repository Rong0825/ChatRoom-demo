...
public class FriendList {
    ...
    // 存储所有群名称以及群好友
    private Map<String,Set<String>> groupList1 = new ConcurrentHashMap<>();
                ...
                if (in.hasNextLine()) {
                    ...
                    if (strFromServer.startsWith("{")) {
                        ...
                        if (messageVO.getType().equals("2")) {
                            ...
                            // 若此群名称在群聊列表
                            if (groupList1.containsKey(groupName)) {
                                if (groupChatGUIList.containsKey(groupName)) {
                                    ...
                                    Set<String> names = groupList1.get(groupName);
                                ...
                                Set<String> friends = (Set<String>) CommUtils.json2Object(messageVO.getTo().split("-")[1],
                                        Set.class);
                                groupList1.put(groupName, friends);
                                loadGroupList();
                                // 2.弹出群聊界面
                                GroupChatGUI groupChatGUI = new GroupChatGUI(groupName,
    ...
    private class PrivateLabelAction implements MouseListener {
        private String labelName;//点击的是哪一个标签也就我们的指的frinedmname;

        //通过构造方法把用户加载用户连表的事件加载进来
    ...
    public FriendList(String userName, Set<String> users,
        ...
        frame.setContentPane(friendPanel);
        ...
        daemonThread.setDaemon(true);//后台线程
        daemonThread.start();
        // 创建群组
        createGroupBtn.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                new CreateGropGUI(userName,users,connect2Server,
                        FriendList.this);
            }
        });
    }
    ...
    public void loadGroupList() {
        ...
        groupNamePanel.setLayout(new BoxLayout(groupNamePanel,
                BoxLayout.Y_AXIS));
        JLabel[] labels = new JLabel[groupList1.size()];
        // Map遍历
        Set<Map.Entry<String,Set<String>>> entries = groupList1.entrySet();
